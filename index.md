...
<table>
    ...
    <tr>
        <td>延时</td><td>计数器：<code>T00-T99</code></td><td><code>空格</code>：让 Txx 计时 00.00 秒-99:59 时<code>=</code>/<code>+</code>/<code>-</code>：给 Txx 赋值 00.00 秒-99:59 时<br /><code>:</code>：让 Txx 计时 DTxx，人机界面的延时</td>
    </tr>
</table>
...
<table>
    ...
    <tr>
        <td rowspan="2">输出</td><td>端口：<code>Y00-Y31</code></td><td><code>闭合</code>：开关接通，并自锁；<br /><code>断开</code>：开关断开，并自锁；<br /><code>翻转</code>：开关接通/断开切换；</td>
    </tr>
    <tr>
        <td>程序组：<code>Z00-Z17</code></td><td><code>运行</code>：从当前位置启动工作；<br /><code>停止</code>：停在程序的开始处；<br /><code>暂停</code>：停在当前位置；</td>
    </tr>
    <tr>
        <td rowspan="2">延时</td><td>立即数：<code>00.00 秒-99:59 时</code></td><td>立即数以 0.01 秒精度倒计时；</td>
    </tr>
    <tr>
        <td>定时器：<code>T00-T59</code></td><td><code>空格</code>：定时器 Txx 以 0.01 秒精度倒计时；<br /><code>=</code>/<code>+</code>/<code>-</code>：定时器 Txx 通过 <code>=</code>/<code>+</code>/<code>-</code> 方式赋值；</td>
    </tr>
    <tr>
        <td>计数</td><td>计数器：<code>C00-C99</code></td><td><code>计数器 Cxx 以 1 次为单位计数；<br /><code>=</code>/<code>+</code>/<code>-</code>：计数器 Cxx 以 <code>=</code>/<code>+</code>/<code>-</code> 方式赋值；</code></td>
    </tr>
    <tr>
        <td>如果</td><td>端口：<code>X00-X31</code><br />端口：<code>Y00-Y31</code></td><td><code>闭合</code>/<code>断开-向下</code>：如果条件成立，程序向下，否则等待；<br /><code>闭合</code>/<code>断开->xxx步</code>：如果条件成立，程序转 xxx 步，否则向下执行；</td>
    </tr>
    <tr>
        <td>循环</td><td><code>xxx 行 xxxx 次</code></td><td>无条件跳转到 xxx 行 xxxx 次</td>
    </tr>
</table>
...
<table>
    ...
    <tr>
        <td>校时</td>
        <td><code>20xx年xx月xx日，xx时xx分xx秒</code></td>
        <td>按北京时间校准，星期x会自动生成，有纽扣电池供电保持。</td>
    </tr>
    <tr>
        <td>定时</td>
        <td><code>xx时xx分xx秒-xx时xx分xx秒</code><br /><code>星期0123456</code></td>
    ...
    <tr>
        <td>与</td>
        <td>端口：<code>X00-X31</code><br />端口：<code>Y08-Y31</code></td>
        <td><code>闭合</code>：开关接通；<br /><code>断开</code>：开关断开；</td>
    </tr>
    <tr>
        <td rowspan="2">输出</td>
    ...
    <tr>
        ...
        <td><code>运行</code>：时段内运行，时段外不受控制；<br /><code>停止</code>：时段内停止，时段外不受控制；
    </tr>
</table>
...
2. 将 V- 与X00-X07 逐个短路一下，对应的输入指示灯点亮，Y00-Y07 跟随闭合，有继电器动作的声音，并且对应输出指示灯点亮，说明输入输出通路正常。
...
|按 $\leftarrow$，$\rightarrow$ 翻阅程序内容|按 $\leftarrow$，$\rightarrow$ 左右移动光标|
|按 `+`，`-` 切换当前显示组|按 $\uparrow$，$\downarrow$ 上下移动光标|
...
如果 X04 没有闭合，先断开 Y02，再等待 X05 闭合，再断开 Y00-01。
...
3. 一组定时必须定时在先，动作在后。如左边程序，一种颜色代表一组定时。
...
如左边程序，不同颜色的两行构成一个逻辑段，不同逻辑段不分先后顺序。
...
"步进"、"运行/停止"、"暂停/运行" 都需要设置对应的输入按钮为 "上跳" 或 "下跳"，否则按一下按钮会执行多次指令。
...
上电运行和 C00 都是带掉电记忆功能的，因此产品上电后，会从断电时的状态继续运行，并且次数也会累计。
...
C00=0000005 次是一个状态，持续时间可能超过 1 秒，那样的话，会导致 Z00 被再次触发，因此在 C00 再等于 5 次多计数一次，避免重复触发 Z00 运行。
...
人机界面通过 DTxx 来向编程界面传递延时时间。后面空格会自动跟踪先前当前 T00, T01 的时间。
